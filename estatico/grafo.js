...
(function () {
  ...
  lienzo.addEventListener('pointerdown', function (e) {
    if (e.pointerType === 'mouse' && [0, 1, 2].indexOf(e.button) === -1) return;
    e.preventDefault();
    arrastrando = true; movido = 0;
    botonArrastre = e.button;
    tipoArrastre = e.pointerType === 'mouse' && e.button === 0
      ? 'girar' : 'mover';
  ...
  lienzo.addEventListener('pointermove', function (e) {
    ...
    if (arrastrando) {
      ...
      ux = p.x; uy = p.y;
    } else if (e.pointerType === 'mouse' && !fijado) {
  ...
  lienzo.addEventListener('pointerup', function (e) {
    ...
    var puedeSeleccionar = e.pointerType !== 'mouse' || botonArrastre === 0;
    if (movido < 6 && puedeSeleccionar) {
      var p = xy(e), n = buscar(p.x, p.y);
      if (n) {
        seleccionar(fijado === n ? null : n);
      } else {
        seleccionar(null);
      }
    }
  ...
  lienzo.addEventListener('touchmove', function (e) {
    if (e.touches.length === 2) {
      var d = Math.hypot(e.touches[0].clientX - e.touches[1].clientX,
                         e.touches[0].clientY - e.touches[1].clientY);
      camZ = Math.max(500, Math.min(2400, camZ * (d0 / d)));
      d0 = d;
    }
  }, { passive: true });
